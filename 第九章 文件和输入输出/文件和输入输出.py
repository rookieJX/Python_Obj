...
不经常出现
# 9.9 永久存储模块
大部分永久性存储模块是用来储存字符串数据的，但是也有方法来归档Python对象
    | pickle 和 marshal 模块
    Pytho提供了许多可以实现最小化永久性储存的模块，其中( pickle 和 marshal)可以用来转换并储存Python对象。该过程将比基本类型复杂的对象转换为一个二进制数据集合，这样就可以把数据集合保存起来或通过网络发送，然后再回复 ------- 数据扁平化、数据序列化、数据顺序花
    | DBM 风格的模块
    *db*系列的模块使用传统的DBM格式写入数据，Python提供了DBM的多种实现：dbhash/bsddb、dbm、gdbm和dumbdbm等。但是最好使用anybdm模块，他会自动检测系统上已经暗转的DBM兼容模块，并选择最好的饿一个
    | shelve模块
    允许对数据库文件进行并发的读访问，但不允许共享读/写访问。
# 9.10 相关模块
